...
class PDOManager {
    ...
    public function getPdo() {
        ...
        // La classe Config a pour mission de nous transmettre les informations de connexion à la BDD. Donc on l'include ici, on l'instancie, et grâce à sa fonction getParametersConnect() (voir fichier app/config.php) on récupère les infos de $connect.

        $this ->  pdo = new PDO('mysql:host=' . $connect['host'] . ';dbname=' . $connect['dbname'], $connect['login'], $connect['password'], array(
            PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING,
            PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8',
            PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC // fetch automatiquement en associative
        ));
